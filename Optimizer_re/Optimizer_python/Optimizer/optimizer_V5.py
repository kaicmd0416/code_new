...
class Optimizer_python:
    ...
    def optimizer_args_processing(self,optimizer_args,portfolio_name):
        inputpath_modedic = glv.get('backtest_config')
        df_mode_dic = pd.read_excel(inputpath_modedic,sheet_name='portfolio_info')
        slice_df_mode_dic=df_mode_dic.iloc[df_mode_dic[df_mode_dic['portfolio_name']==portfolio_name].index]
    ...
    def data_processing_main(self,optimizer_args, outpath_optimizer_python):
        portfolio_name=optimizer_args.get('portfolio_name')
        style_list,industry_list=valid_factor_withdraw(portfolio_name)
    ...
    def main_optimizer(self,portfolio_name):
