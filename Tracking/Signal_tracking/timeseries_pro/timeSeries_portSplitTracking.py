...
def ScoreTracking_timeSeries_main_port(portfolio_name_list):
    inputpath_original=glv.get('output_portfolio_split')
    gt.folder_creator2(inputpath_original)
    inputpath=os.path.join(inputpath_original,'portflioSplit_excessReturn.xlsx')
    inputpath2=os.path.join(inputpath_original,'portflioSplit_excessNetvalue.xlsx')
    #判断是否有文件
        ...
        with pd.ExcelWriter(inputpath, mode=mode, engine='openpyxl',
            ...
            for portfolio_name in portfolio_name_list:
                ...
                if len(running_date_list) > 0:
                    ...
                    df2.to_excel(writer2, sheet_name=portfolio_name, index=False)
    else:
        print('portflioSplit_excessReturn和portflioSplit_excessNetvalue已经更新到最新日期:' + str(end_date))
