...
def MktData_update_main(start_date,end_date,is_sql):
    ...
    for available_date in working_days_list:
        ...
        FDU=futureData_update(available_date,available_date)
        ODU=optionData_update(available_date,available_date)
        EDU=etfData_update(available_date,available_date)
        LAU=lhb_amt_update_main(available_date,available_date)
        NLU= nlb_update_main(available_date,available_date)
        FDDU=futureDifference_update_main(available_date,available_date)
...
def CBData_update_main(start_date,end_date,is_sql):
    working_days_list = gt.working_days_list(start_date, end_date)
    for available_date in working_days_list:
        CBU = cbData_update(available_date, available_date,is_sql)
...
def MktData_update_main2(start_date,end_date,is_sql):
    ...
    for available_date in working_days_list:
        ...
        #LAU = lhb_amt_update_main(available_date, available_date,is_sql)
        #NLU = nlb_update_main(available_date, available_date,is_sql)
        #FDDU = futureDifference_update_main(available_date, available_date,is_sql)
        #SDU.stock_data_update_main()
        #IRU.index_data_update_main()
        #ICU.index_component_update_main()
        #FDU.future_data_update_main()
...
if __name__ == '__main__':
    ...
    MktData_update_main2('2025-01-03', '2025-05-09',True)
