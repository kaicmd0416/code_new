...
class product_tracking:
    ...
    def partial_analysis(self):
        ...
        # 处理详细数据
        df_detail = df_detail[['valuation_date','code', 'quantity', 'delta', 'risk_mkt_value', 'profit', 'portfolio_name']]
    ...
    def product_info_processing(self):
        ...
        df_info['total_profit']=df_info['stock_profit']+df_info['indexfuture_profit']+df_info['commfuture_profit']+df_info['option_profit']+df_info['etf_profit']+df_info['cb_profit']+df_info['bond_profit']
        df_info['total_mktvalue'] = df_info['stock_mktvalue'] + df_info['indexfuture_mktvalue'] + df_info[
            'commfuture_mktvalue'] + df_info['option_mktvalue'] + df_info['etf_mktvalue'] + df_info['cb_mktvalue'] + df_info[
                                      'bond_mktvalue']
        df_info['proportion_stock'] = df_info['stock_mktvalue']/df_info['NetAssetValue']
        df_info['proportion_option'] = df_info['option_mktvalue'] / df_info['NetAssetValue']
        df_info['proportion_indexfuture'] = df_info['indexfuture_mktvalue'] / df_info['NetAssetValue']
        df_info['proportion_commfuture'] = df_info['commfuture_mktvalue'] / df_info['NetAssetValue']
        df_info['proportion_cb'] = df_info['cb_mktvalue'] / df_info['NetAssetValue']
        df_info['proportion_etf'] = df_info['etf_mktvalue'] / df_info['NetAssetValue']
        df_info['leverage_ratio']=df_info['total_mktvalue']/df_info['NetAssetValue']
        df_info['product_return']=df_info['total_profit']/df_info['NetAssetValue_yes']
    ...
    def productTracking_main(self):
        ...
        """
        # 获取产品信息
        df_output,df_output2 = self.product_info_processing()
        # 获取交易行为数据
...
if __name__ == '__main__':
    pt=product_tracking('2025-08-22','2025-08-25','SGS958',False)
